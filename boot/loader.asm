...
InitKernel: ;遍历每一个 Program Header，根据 Program Header 中的信息来确定把什么放进内存，放到什么位置，以及放多少。
	xor esi, esi
	mov cx, word [BaseOfKernelFilePhyAddr + 2Ch] ;ecx <- pELFHdr->e_phnum [program header table 中的表项数目]
	movzx ecx, cx 
	mov	esi, [BaseOfKernelFilePhyAddr + 1Ch]	; esi <- pELFHdr->e_phoff【rogram header table在文件中的偏移】
	add	esi, BaseOfKernelFilePhyAddr		; esi <- OffsetOfKernel + pELFHdr->e_phoff
.Begin:
	mov eax, [esi + 0]
	cmp eax, 0 
	jz .NoAction
	push dword[esi + 010h]; file size 
	mov eax, [esi + 04h] ; 相对文件的偏移
	add eax, BaseOfKernelFilePhyAddr
	push eax ; memcpy的src被压入参数
	push dword [esi + 08h] ; memcpy的目标地址
	call MemCpy
	add esp, 12 
.NoAction:
	add esi, 020h ;020h是一个program header table表项的大小
	dec ecx 
	jnz .Begin 
